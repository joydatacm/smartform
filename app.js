...
function renderForm() {
  ...
  const layout = el("div", { class: "form-layout" });
  ...
  // Build the section nav from what actually got rendered (respects
  // whichever style variant is currently selected), not a static schema walk.
  const navTargets = Array.from(body.querySelectorAll("[id^='navsec-']"));
  if (navTargets.length > 1) {
    const nav = el("nav", { class: "section-nav" });
    nav.appendChild(el("div", { class: "section-nav-label" }, "Jump to"));
    for (const target of navTargets) {
      const titleEl = target.querySelector(".section-title");
      const title = titleEl ? titleEl.textContent : target.id;
...
const NAV_CATEGORY_NAMES = ["TOP", "GRAPHICS", "PAPER", "PRINTING", "BINDERY", "LETTERSHOP", "SHIPPING", "ADDITIONAL"];
...
function renderSection(node, pathParts, idx) {
  ...
  if (NAV_CATEGORY_NAMES.includes(node.name)) {
    box.id = "navsec-" + path.join("-").replace(/[^a-zA-Z0-9-]/g, "");
  }
